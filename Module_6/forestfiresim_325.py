...
WIDTH = 79
HEIGHT = 22
LAKE_WIDTH = range(22, 54)  # Creat lake width
LAKE_HEIGHT = range(8, 14,)  # Create lake height

TREE = 'A'
FIRE = '@'
WATER = '~' # Declare lake constant
EMPTY = ' '
        ...
        for x in range(forest['width']):
            for y in range(forest['height']):
                ...
                if forest[(x, y)] == WATER:
                    ...
                    for ix in range(-1, 2):
                        for iy in range(-1, 2):
                            # Fire spreads to neighboring trees and not through water
...
def createNewForest():
    ...
    for x in LAKE_WIDTH:
        for y in LAKE_HEIGHT:
            forest[(x, y)] = WATER
    # Fill the rest with trees or empty spaces
    for x in range(WIDTH):
        for y in range(HEIGHT):
            if (x, y) not in forest:  # Skip lake cells, since they already have a value assigned to them
